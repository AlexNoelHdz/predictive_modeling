...
# K_x =  polynomial_kernel(X,degree=3,coef0=0) # K(x, x*) = (gamma <x, x*> + coef0)^degree
# K_x =  rbf_kernel(X,gamma=0.01) # K(x, x*) = exp(-gamma ||x-x*||^2)

#%% Review the min and max values in the dataset
print('Data\t min_val \t max_val\n X\t\t %0.4g \t %0.4g\n Kernel\t %0.4g \t %0.4g'%(np.min(X),np.max(X),np.min(K_x),np.max(K_x)))
...
plt.subplot(1,2,1)
plt.scatter(X, y, c='b', label='data')
plt.scatter(X, yrnd, c='r', s=20, label='data rnd',zorder=2)
plt.scatter(X[sv_x], yrnd[sv_x], c='k',s=80, marker='s', label='SVR support vectors', zorder=1,edgecolors=(0, 0, 0))
